...
## 다음 할 일
- Phase 1-3: 로그인·토큰(FND-003). JWT 발급/검증을 순수 로직(`auth.domain`, jjwt + `Clock` 주입, 단위 테스트로 만료·위조·클레임 검증)으로 분리하고, `AuthService`(로그인: loginId+비밀번호 검증→Access+Refresh 발급, BCrypt 매칭, 비활성 거부) + `RefreshToken` 엔티티/마이그레이션(`V3`) + 로그인/토큰 갱신 컨트롤러를 작성한다. Access 토큰에는 사용자 식별자+만료만 포함(역할 제외, FND-003 AC4). 최초 로그인 비밀번호 변경 강제 플래그 반영. verify: JWT 순수 로직 단위 테스트 `gradlew test` 통과. (로그인 통합 테스트는 DB 필요 → @Disabled, OPEN[01].)
