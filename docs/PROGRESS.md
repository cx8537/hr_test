...
## 다음 할 일
- Phase 1-2: FND 엔티티·영속성. `Employee`/`Department`/`Role`(또는 코드 enum 재사용)/`EmployeeRoleScope` JPA 엔티티(`com.example.hr.<모듈>.entity`)와 Flyway 마이그레이션 `V2__fnd_core.sql`(테이블+제약: emp_no/login_id/dept_code 유니크, dept level≤3, status CHECK, version, KST 타임스탬프) 작성. 최초 시스템관리자 시드(`BOOTSTRAP_ADMIN_*`, must_change_password=true) 포함. Repository + 통합 테스트(로컬 PostgreSQL 테스트 스키마). verify: 엔티티↔마이그레이션 정합(`ddl-auto=validate`), Repository 통합 테스트 통과. (DB 자격증명 필요 — 미확보 시 엔티티/SQL 작성까지 하고 통합 테스트는 OPEN[01]에 묶어 보류.)
